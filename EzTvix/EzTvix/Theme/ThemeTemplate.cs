...
namespace EzTvix.Theme
{
    ...
    class ThemeTemplate
    {
        ...
        public ThemeTemplate(String _theme)
        {
            loadConfig();
            this.Name = _theme;
        }
       ...
       //     _node = xmlConfig.SelectSingleNode("//VideoBox/Mask");
            ...
            _node = xmlConfig.SelectSingleNode("//Item[@name='" + ItemList[ItemType.Folder].Name + "']/TextZone");
            ItemList[ItemType.Folder].Text.TextZone = new Rectangle(
                            Int32.Parse(_node.Attributes["x"].Value),
                            Int32.Parse(_node.Attributes["y"].Value),
                            Int32.Parse(_node.Attributes["width"].Value),
                            Int32.Parse(_node.Attributes["height"].Value));
            ...
            ItemList.Add(ItemType.VideoCover, new ImageTemplate(@"\VideoCover.png", this._currentTheme, Layer.Background));//ItemList[ItemType.VideoCover].Name = "VideoCover.png";
            _node = xmlConfig.SelectSingleNode("//Item[@name='" + ItemList[ItemType.VideoBox].Name + "']/Mask");
            //     _node = xmlConfig.SelectSingleNode("//VideoBox/Mask");

            ItemList[ItemType.VideoCover].layerToSave = Layer.Background;

            ItemList[ItemType.VideoCover].Background.Mask = new Rectangle(
                Int32.Parse(_node.Attributes["x"].Value),
                Int32.Parse(_node.Attributes["y"].Value),
                Int32.Parse(_node.Attributes["width"].Value),
                Int32.Parse(_node.Attributes["height"].Value));
        ...
        #endregion
        public void ApplyBackgroundVideo(Movie movie, String path, bool saveXml = true)
        {
            this.BackgroundVideo.GenerateFast(movie, path);
            this.SaveVideoBox(path);
            if (ThemeInfo.UpFolderExist)
                this.SaveUpFolderIcon(path);
            //save movie as xml
            if (saveXml)
                movie.SaveAsXml(path);

        }
        public void Apply(String path)
        {
            string fullName = path + ThemeInfo.VideoBackgroundFullName;
            if (File.Exists(fullName)) File.Delete(fullName);

            this.SaveVideoBackground(path);
            //Bitmap bmp = new Bitmap(this.Background.BaseImage);
            //bmp.Save(path + ThemeInfo.VideoBackgroundName + "." + ThemeInfo.VideoBackgroundExtention);

            if (ThemeInfo.UpFolderExist)
                this.SaveUpFolderIcon(path);
            //this[ItemType.FolderUp].Image.Save(path + ThemeInfo.upFolderIconFullName);

            //if (! File.Exists( path + ThemeInfo.FolderIconFullName))
                this.SaveFolderIcon(path);
                //this[ItemType.Folder].Image.Save(path + ThemeInfo.FolderIconFullName);

//            bmp.Dispose();
        }
        public void SaveVideoBox(string path)
        {
            string fullName = path + ThemeInfo.VideoBoxFullName;
            if (File.Exists(fullName)) File.Delete(fullName);

            Bitmap dummy = new Bitmap(this[ItemType.VideoBox].Image);
            dummy.Save(fullName, ThemeInfo.VideoBoxFormat);
            dummy.Dispose();
        }
        public void SaveVideoBackground(string path)
        {
            string fullName = path + ThemeInfo.VideoBackgroundFullName;
            if (File.Exists(fullName)) File.Delete(fullName);

            Bitmap dummy = new Bitmap(this.Background.BaseImage);
            dummy.Save(fullName, ThemeInfo.VideoBackgroundFormat);
            dummy.Dispose();
        }
        public void SaveUpFolderIcon(string path)
        {
            string fullName = path + ThemeInfo.upFolderIconFullName;
            if (File.Exists(fullName)) File.Delete(fullName);

            Bitmap dummy = new Bitmap(this[ItemType.FolderUp].Image);
            dummy.Save(fullName, ThemeInfo.upFolderIconFormat);
            dummy.Dispose();

        }
        public void SaveFolderIcon(string path)
        {
            this.SaveFolderIcon(this[ItemType.Folder].Image, path);
        }
        public void SaveFolderIcon(Image img, string path)
        {
            string fullName = path + ThemeInfo.FolderIconFullName;
            if (File.Exists(fullName)) File.Delete(fullName);

            Bitmap dummy = new Bitmap(img);
            dummy.Save(fullName, ThemeInfo.FolderIconFormat);
            dummy.Dispose();
