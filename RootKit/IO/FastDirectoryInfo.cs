...
namespace RootKit.IO
{
    ...
    /// </remarks>
    public static class FastDirectoryInfo
    {
        /// <summary>
        ...
        {
            return FastDirectoryInfo.EnumerateFiles(path, "*");
        }
        ...
        {
            return FastDirectoryInfo.EnumerateFiles(path, searchPattern, SearchOption.TopDirectoryOnly);
        }

        /// <summary>
        /// Gets <see cref="FastFileInfo"/> for all the files in a directory that 
        ...
        /// <paramref name="filter"/> is a null reference (Nothing in VB)
        /// </exception>
        public static FastFileInfo[] GetFiles(string path, string searchPattern, SearchOption searchOption)
        {
            IEnumerable<FastFileInfo> e = FastDirectoryInfo.EnumerateFiles(path, searchPattern, searchOption);
            List<FastFileInfo> list = new List<FastFileInfo>(e);

            FastFileInfo[] retval = new FastFileInfo[list.Count];
            list.CopyTo(retval);
        ...
        private class FileEnumerator : IEnumerator<FastFileInfo>
        {
            ...
            {
                get { return new FastFileInfo(m_path, m_win_find_data); }
            }
