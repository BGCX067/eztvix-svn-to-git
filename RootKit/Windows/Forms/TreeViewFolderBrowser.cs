...
namespace RootKit.Windows.Forms
{
    ...
    [System.Drawing.ToolboxBitmap(typeof(TreeViewFolderBrowser))]   
   public class TreeViewFolderBrowser : TreeView
    ...
    // [System.Drawing.ToolboxBitmap(typeof(System.Windows.Forms.TreeView))]
        ...
        public void ShowFolder(string _directoryPath)
        {
            this.ShowFolder(_directoryPath, false);
        }
        /// <summary>
        /// Focus the specified folder and scroll it in to view.
        /// </summary>
        /// <param name="_directoryPath">The path which should be focused</param>
        /// <param name="_expandLastTreeNode">Bool to auto expand content of selected treenode</param>
        public void ShowFolder(string _directoryPath, bool _expandLastTreeNode)
        {
            if ((_directoryPath == null) || (_directoryPath == "") || (_directoryPath == string.Empty)) return;
            // start search at root node
            TreeNodeCollection nodeCol = _dataProvider.RequestDriveCollection(_helper);
            //
            if (!System.IO.Directory.Exists(_directoryPath) || nodeCol == null) return;
            //
            System.IO.DirectoryInfo dirInfo = new System.IO.DirectoryInfo(_directoryPath);
            // get path tokens
                    ...
                    foreach (TreeNodePath n in nodeCol)
                    {
                        if (n.Path.ToLower().CompareTo(dirs[i].ToString().ToLower()) == 0)
                        {
                            ...
                            if (i == 0)
                            {
                                n.EnsureVisible();
                                this.SelectedNode = n;
                                if (_expandLastTreeNode) n.Expand();
                            }
                            else
                            {
                                n.Expand();
    ...
    /// </summary>
    public class TreeNodePath : System.Windows.Forms.TreeNode
    {
        #region fields
    ...
    /// </summary>
    [Flags]
    [Editor(typeof(RootKit.Windows.Forms.Design.FlagsEditor), typeof(UITypeEditor))]
